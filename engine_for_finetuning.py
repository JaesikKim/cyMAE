...
def train_one_epoch(args, model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if (log_writer is not None) and utils.is_main_process():
...
def evaluate(args, data_loader, model, device):
    ...
    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
    print('* Acc@1 {top1.global_avg:.3f}  Bacc {bacc.global_avg:.3f}  ARI {ari.global_avg:.3f}  NMI {nmi.global_avg:.3f}  loss {losses.global_avg:.3f}'
          .format(top1=metric_logger.acc1, bacc=metric_logger.bacc, ari=metric_logger.ari, nmi=metric_logger.nmi, losses=metric_logger.loss))
    #print('* Acc@1 {top1.global_avg:.3f} Acc@5 {top5.global_avg:.3f} loss {losses.global_avg:.3f}'
    #      .format(top1=metric_logger.acc1, top5=metric_logger.acc5, losses=metric_logger.loss))

    print(confs)
    print(confs.diagonal()/confs.sum(axis=1))
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}
